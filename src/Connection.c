...
void do_recv(struct connection *c)
{
	...
	for(;;)
	{
		int bytestransfer = WSARecv(c->socket,&c->recv_overlap.m_super,RECV_NOW);
		if(bytestransfer == 0)
			break;
...
int32_t connection_send(struct connection *c,wpacket_t w,packet_send_finish callback)
{
	...
	if(!c->send_overlap.isUsed)
	{
		...
		if(O)
		{
			...
			return WSASend(c->socket,O,SEND_POST);
		}
	}
	return -1;
}
...
int32_t connection_start_recv(struct connection *c)
{
	...
	c->recv_overlap.m_super.iovec = c->wrecvbuf;
	uint32_t err_code;
	return WSARecv(c->socket,&c->recv_overlap.m_super,RECV_POST);
}
